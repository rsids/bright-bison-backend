<?php
 ...
 /* o------------------------------------------------------------------------------o
    ...
    *    - Correctly strip accented suffixes (e.g. industriÃ«le)
    *
    ...
    * o------------------------------------------------------------------------------o */
    ...
    class nl_stemmer{
        ...
        public function stem($word) {
            ...
            // Remove accents
            $word = str_replace(array('Ã¤', 'Ã«', 'Ã¯', 'Ã¶', 'Ã¼', 'Ã¡', 'Ã©', 'Ã­', 'Ã³', 'Ãº'),
                                array('a', 'e', 'i', 'o', 'u', 'a', 'e', 'i', 'o', 'u'),
                                $word);

            // Put initial y, y after a vowel, and i between vowels into upper case (treat as consonants).
            $word = preg_replace(array('/^y|(?<=[aeiouyÃ¨])y/u', '/(?<=[aeiouyÃ¨])i(?=[aeiouyÃ¨])/u'),
                                array('Y', 'I'),
                                $word);
            ...
            if (preg_match('/[aeiouyÃ¨][^aeiouyÃ¨]/u', $word, $matches, PREG_OFFSET_CAPTURE)) {
                $r1 = $matches[0][1] + 2;
            }
            ...
            if (preg_match('/[aeiouyÃ¨][^aeiouyÃ¨]/u', $word, $matches, PREG_OFFSET_CAPTURE, $r1)) {
                $r2 = $matches[0][1] + 2;
            }
        ...
        function dutchstemmer_step0($word) {
            ...
            return preg_replace('/eÃ«n$/u', 'e', preg_replace('/(ieel|iÃ«le|ieÃ«n)$/u', 'ie', $word));
        }
        ...
        function dutchstemmer_step1($word, $r1, $r2) {
            ...
            if ($r1) {
                ...
                // -en(e)
            else if (preg_match('/(?<=[^aeiouyÃ¨]|gem)ene?$/u', $word, $matches, 0, $r1)) {
                return self::dutchstemmer_undouble(preg_replace('/ene?$/u', '', $word, -1, $count));
            }
            // -s(e)
            else if (preg_match('/(?<=[^jaeiouyÃ¨])se?$/u', $word, $matches, 0, $r1)) {
        ...
        function dutchstemmer_step2($word, $r1, $r2) {
            ...
            if ($r1) {
                if (preg_match('/(?<=[^aeiouyÃ¨])e$/u', $word, $matches, 0, $r1)) {
        ...
        function dutchstemmer_step4($word, $r1, $r2) {
            ...
            if (preg_match('/[^aeiouyÃ¨](aa|ee|oo|uu)[^IaeiouyÃ¨]$/u', $word)) {
