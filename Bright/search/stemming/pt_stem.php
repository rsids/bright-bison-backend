    ...
    * o------------------------------------------------------------------------------o */
    class pt_stemmer {
        ...
        function removeAccent($word) {
            //return $word;
            return str_replace( array('á','é','í','ó','ú','â','ê','ô','ç','ã','õ','ü','ñ'),
        ...
        function stem($word) {
            ...
            // ã and õ should be treated as a vowel followed by a consonant
            ...
            if (($suf = pt_stemmer::endsinArr($r2_txt, array('amentos', 'imentos', 'amento', 'imento', 'adoras', 'adores', 'aço~es', 'ismos', 'istas', 'adora', 'aça~o', 'antes', 'ância', 'ezas', 'icos', 'icas', 'ismo', 'ável', 'ível', 'ista', 'osos', 'osas', 'ador', 'ante', 'eza', 'ico', 'ica', 'oso', 'osa'))) != '') {
                $word = substr($word, 0, -strlen($suf));    # rule1
            } elseif (($suf = pt_stemmer::endsinArr($r2_txt, array('logía', 'logías'))) != '') {
                $word = substr($word, 0, -strlen($suf)) . 'log';
            } elseif (($suf = pt_stemmer::endsinArr($r2_txt, array('ución', 'uciones'))) != '') {
                $word = substr($word, 0, -strlen($suf)) . 'u';
            } elseif (($suf = pt_stemmer::endsinArr($r2_txt, array('ência', 'ências'))) != '') {
                ...
                $word = substr($word, 0, -strlen($suf));
            } elseif (($suf = pt_stemmer::endsinArr($r2_txt, array('antemente', 'avelmente', 'ívelmente', 'mente'))) != '') {
                $word = substr($word, 0, -strlen($suf));
            ...
            // Do step 2 if no ending was removed by step 1: now remove verb suffixes
                if (($suf = pt_stemmer::endsinArr($rv_txt, array(   'aríamos', 'eríamos', 'iríamos', 'ássemos', 'êssemos', 'íssemos', 'aríeis', 'eríeis', 'iríeis', 'ásseis', 'ésseis', 'ísseis',
                                                                    'áramos', 'éramos', 'íramos', 'ávamos', 'aremos', 'eremos', 'iremos', 'ariam', 'eriam', 'iriam', 'assem', 'essem', 'issem',
                                                                    'ara~o', 'era~o', 'ira~o', 'arias', 'erias', 'irias', 'ardes', 'erdes', 'irdes', 'asses', 'esses', 'isses', 'astes', 'estes', 'istes',
                                                                    'áreis', 'areis', 'éreis', 'ereis', 'íreis', 'ireis', 'áveis', 'íamos', 'armos', 'ermos', 'irmos', 'aria', 'eria', 'iria',
                                                                    'asse', 'esse', 'isse', 'aste', 'este', 'iste', 'arei', 'erei', 'irei', 'aram', 'eram', 'iram', 'avam', 'arem', 'erem', 'irem',
                                                                    'ando', 'endo', 'indo', 'adas', 'idas', 'arás', 'aras', 'erás', 'eras', 'irás', 'avas', 'ares', 'eres', 'ires', 'íeis',
                                                                    'ados', 'idos', 'ámos', 'amos', 'emos', 'imos', 'iras', 'ada', 'ida', 'ará', 'ara', 'erá', 'era', 'irá', 'ava', 'iam',
            ...
            if ($stem != $word_orig) {
                ...
                // Step 4 conditioned
                if (($suf = pt_stemmer::endsinArr($rv_txt, array('os', 'a', 'i', 'o', 'á', 'í', 'ó'))) != '') {
                    $word = substr($word, 0, -strlen($suf));
                    $r1_txt = substr($word, $r1);
                    $r2_txt = substr($word, $r2);
                    $rv_txt = substr($word, $rv);
            ...
            if (($suf = pt_stemmer::endsinArr($rv_txt, array(' cie', ' cié', ' ciê', 'gue', 'gué', 'guê', 'e', 'é', 'ê'))) != '') {
                $word = substr($word, 0, -strlen($suf));
            }

            $word = str_replace('a~', 'ã', $word);
            $word = str_replace('~o','õ', $word);
