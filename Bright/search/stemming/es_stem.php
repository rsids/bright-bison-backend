   ...
   *  Adapted for Sphider-plus application by Rolf Kellner [Tec] Feb. 2010
    ...
    * o------------------------------------------------------------------------------o */
    ...
    class es_stemmer {
        ...
        function is_vowel($c) {
            return ($c == 'a' || $c == 'e' || $c == 'i' || $c == 'o' || $c == 'u' || $c == 'Ã¡' || $c == 'Ã©' ||
                $c == 'Ã­' || $c == 'Ã³' || $c == 'Ãº');
        }
        ...
        function removeAccent($word) {
            return str_replace(array('Ã¡','Ã©','Ã­','Ã³','Ãº'), array('a','e','i','o','u'), $word);
        }
        ...
        public function stem($word) {
            ...
            $pronoun_suf_pre1 = array('Ã©ndo', 'Ã¡ndo', 'Ã¡r', 'Ã©r', 'Ã­r');
            ...
            if (($suf = es_stemmer::endsinArr($r2_txt, array('anza', 'anzas', 'ico', 'ica', 'icos', 'icas', 'ismo', 'ismos', 'able', 'ables', 'ible', 'ibles', 'ista', 'istas', 'oso', 'osa', 'osos', 'osas', 'amiento', 'amientos', 'imiento', 'imientos'))) != '') {
                $word = substr($word, 0, -strlen($suf));
            } elseif (($suf = es_stemmer::endsinArr($r2_txt, array('icadora', 'icador', 'icaciÃ³n', 'icadoras', 'icadores', 'icaciones', 'icante', 'icantes', 'icancia', 'icancias', 'adora', 'ador', 'aciÃ³n', 'adoras', 'adores', 'aciones', 'ante', 'antes', 'ancia', 'ancias'))) != '') {
                $word = substr($word, 0, -strlen($suf));
            } elseif (($suf = es_stemmer::endsinArr($r2_txt, array('logÃ­a', 'logÃ­as'))) != '') {
                $word = substr($word, 0, -strlen($suf)) . 'log';
            } elseif (($suf = es_stemmer::endsinArr($r2_txt, array('uciÃ³n', 'uciones'))) != '') {
            ...
            if ($word_after0 == $word_after1) {
                ...
                if (($suf = es_stemmer::endsinArr($rv_txt, array('ya', 'ye', 'yan', 'yen', 'yeron', 'yendo', 'yo', 'yÃ³', 'yas', 'yes', 'yais', 'yamos'))) != '' && (substr($word,-strlen($suf)-1,1) == 'u')) {
                    $word = substr($word,0, -strlen($suf));
                }
            ...
            if (($suf = es_stemmer::endsinArr($rv_txt, array('os', 'a', 'o', 'Ã¡', 'Ã­', 'Ã³'))) != '') {
                $word = substr($word,0, -strlen($suf));
            } elseif (($suf = es_stemmer::endsinArr($rv_txt ,array('e','Ã©'))) != '') {
                $word = substr($word, 0 ,-1);
                $rv_txt = substr($word, $rv);
