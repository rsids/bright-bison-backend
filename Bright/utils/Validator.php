...
class Validator {
	...
	/**
	 * Checks if the string is a valid emailAddress
	 * @param string $emailAddress The string to check
	 * @return boolean True when valid
	 */
	public function checkEmail($emailAddress) {
		...
		return preg_match("/^[A-Za-z0-9_+-]+([\.]{1}[A-Za-z0-9_+-]+)*@[A-Za-z0-9-]+(\.[A-Za-z0-9-]+)*(\.[A-Za-z0-9-]{2,6})+$/", $emailAddress);
	}
