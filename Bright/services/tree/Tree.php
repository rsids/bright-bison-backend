...
ORDER BY `index`";
    ...
    private function _getSibling($treeId, $orderfield = 'index', $orderdir = 1, $dir = 'next') {
        ...
        if(!$treeId)
            throw new Exception("Invalid treeId");
        if(!$orderdir)
    ...
    public function setLocked($treeId, $locked) {
        ...
        if(!is_numeric($treeId))
            throw $this -> throwException(2002);
        if(!is_bool($locked))
            throw $this -> throwException(2005);
    ...
    public function generateSitemap() {
        ...
        foreach($result as $page) {
            ...
            switch($life) {
                case 'yea': $lifetime = 'yearly'; break;
                case 'mon': $lifetime = 'monthly'; break;
                case 'wee': $lifetime = 'weekly'; break;
                case 'day': $lifetime = 'daily'; break;
                case 'hou': $lifetime = 'hourly'; break;
                case 'min': $lifetime = 'hourly'; break;
            }
        ...
        try {
            ...
            throw $this -> throwException(Exceptions::CANNOT_CREATE_SITEMAP);
        }
    }
