...
class Content extends Permissions {
	...
	public function generateLabel($title, $id = 0, $table = 'page') {
		...
		if(count($titlearr) > 1 && is_numeric($titlearr[count($titlearr) - 1])) {
	 ...
	 * @param OPage $page
	 * @param string $table
	 * @throws Exception Exceptions::INCORRECT_PARAM_OBJECT
	 * @return unknown
	 */
	protected function setContent(OPage $page, $table = 'content') {
		...
		if(!isset($page -> content) || $page -> content == null)
			throw new Exception("Exceptions::INCORRECT_PARAM_OBJECT", Exceptions::INCORRECT_PARAM_OBJECT);
