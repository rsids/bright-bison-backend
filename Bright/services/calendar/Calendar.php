...
//		}
	...
	public function getEventsByRange($start = -1, $end = -1, $limit = 0, $offset = 0, $countResults = false, $order = Calendar::ASC, $enabledOnly = false, $filter = null) {
		if(!is_numeric($order))		throw $this -> throwException(Exceptions::INCORRECT_PARAM_INT, '$order');
		if(!is_numeric($start))		throw $this -> throwException(Exceptions::INCORRECT_PARAM_INT, '$start');
		if(!is_numeric($end))		throw $this -> throwException(Exceptions::INCORRECT_PARAM_INT, '$end');
		if(!is_numeric($limit))		throw $this -> throwException(Exceptions::INCORRECT_PARAM_INT, '$limit');
		if(!is_numeric($offset))	throw $this -> throwException(Exceptions::INCORRECT_PARAM_INT, '$offset');
			...
			foreach($additionalFilters as $filter) {
				...
				switch($filter['searchtype']) {
					...
					case 'EQUALS':
						...
						break;
					default:
						throw $this -> throwException(Exceptions::INCORRECT_PARAM_ARRAYVAL);
				}
...
//		$c -> setCache($result, "eventsByIdRange_$argstring", strtotime('+1 year'));
	...
	public function getEventsByIds($ids, $includeContent = false) {
		if(!is_array($ids))
			throw $this -> throwException(Exceptions::INCORRECT_PARAM_ARRAY, 'ids');

		foreach($ids as $id) {
			if(!is_numeric($id))
			throw $this -> throwException(Exceptions::INCORRECT_PARAM_INT, 'calendarId');
		}
	...
	public function getEvent($calendarId) {
		if(!is_numeric($calendarId))
			throw $this -> throwException(Exceptions::INCORRECT_PARAM_INT);
	...
	public function getEventByLabel($label) {
		if(!is_string($label))
			throw $this -> throwException(Exceptions::INCORRECT_PARAM_STRING);
	...
	public function setEvent(OCalendarEvent $event, $executeHook = true, $updateContent = true) {
		...
		if(!isset($event -> dates) || count($event -> dates) == 0) {
			throw $this -> throwException(Exceptions::NOT_ENOUGH_DATES);
		}
					...
					case 'BYDAY':
						...
						switch($freq){
							case 'WEEKLY':
								...
								foreach($days as $day) {
									switch($day) {
										case 'SU': $dayarr[0] = 1; break;
										case 'MO': $dayarr[1] = 1; break;
										case 'TU': $dayarr[2] = 1; break;
										case 'WE': $dayarr[3] = 1; break;
										case 'TH': $dayarr[4] = 1; break;
										case 'FR': $dayarr[5] = 1; break;
										case 'SA': $dayarr[6] = 1; break;
									}
								}
								break;
...
// 									$edow +=7;
	...
	/**
	 * @deprecated 2.0 - Mar 12, 2012
	 */
	private function _cast(OPage $page) {
		trigger_error('This method is deprecated', E_USER_DEPRECATED);
		$cestring = serialize($page);
		...
		return unserialize($cestring);
	}

	/**
	 * @deprecated 2.0 - Mar 12, 2012
	 */
	private function _castBack(OCalendarEvent $page) {
		trigger_error('This method is deprecated', E_USER_DEPRECATED);
		$cestring = serialize($page);
		$cestring = str_replace('O:14:"OCalendarEvent"','O:5:"OPage"', $cestring);
		$cestring = str_replace('s:13:"_explicitType";s:14:"OCalendarEvent"','s:13:"_explicitType";s:5:"OPage"', $cestring);
		return unserialize($cestring);
	}
