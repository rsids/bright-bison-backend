...
class Serve
{
    ...
    public function servepage($nodes)
    {
        ...
        if (!isset($_GET['nc']) && $this->_isCachable()) {
            $cacheurl = $this->_getCacheUrl($nodes);
    ...
    public function serveSpecial($type)
    {
        ...
        switch ($type) {
            ...
            case Serve::$SPECIAL_404:
                ...
                Connection::getInstance()->addTo404Log($bright_log);
    ...
    private function _checkCache($nodes)
    {
        if (!$this->_isCachable()) {
            return false;
        }
    ...
    private function _getCacheUrl($nodes)
    {
        ...
        return 'page%' . $_SESSION['language'] . '%' . join('%', $nodes) . $bright_qstring;
    }
