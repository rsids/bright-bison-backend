...
ob_start();
if (!include_once(dirname(__FILE__) . DIRECTORY_SEPARATOR . 'phpthumb.class.php')) {
	ob_end_flush();
	die('failed to include_once("'.realpath(dirname(__FILE__).DIRECTORY_SEPARATOR .'phpthumb.class.php').'")');
}
ob_end_clean();
...
if (file_exists(dirname(__FILE__).'/phpThumb.config.php')) {
	...
	if (include_once(dirname(__FILE__) . '/phpThumb.config.php')) {
		// great
		$phpThumb -> config_imagemagick_path = $PHPTHUMB_CONFIG['imagemagick_path'];
		foreach($PHPTHUMB_CONFIG as $key => $value) {
			if(isset($phpThumb -> {'config_' . $key})) {
				$phpThumb -> {'config_' . $key} = $value;
			}
		}
	} else {
		ob_end_flush();
...
$phpThumb->DebugMessage('trying to setParameters', __FILE__, __LINE__);
...
function SendSaveAsFileHeaderIfNeeded() {
	...
	global $phpThumb;
	$downloadfilename = phpthumb_functions::SanitizeFilename(@$_GET['sia'] ? $_GET['sia'] : (@$_GET['down'] ? $_GET['down'] : 'bright_thumbnail.'.(@$_GET['f'] ? $_GET['f'] : 'jpg')));
	if (@$downloadfilename) {
