...
class phpthumb {
	...
	public $q    = 90;       // jpeg output Quality
...
//		}
	...
	function OutputThumbnail() {
		...
		$downloadfilename = phpthumb_functions::SanitizeFilename(is_string($this->sia) ? $this->sia : ($this->down ? $this->down : 'bright_thumbnail.'.$this->thumbnailFormat));
	...
	function ResolveFilenameToAbsolute($filename) {
		...
		if ($this->iswindows) {
			// This line gives problems with directories starting with a digit
	...
	function ImageMagickVersion($returnRAW=false) {
		...
		if (is_null($versionstring)) {
			...
			if ($commandline) {
				$versionstr = ' --version';
				$this->DebugMessage('ImageMagick version checked with "'.$commandline.$versionstr.'"', __FILE__, __LINE__);
				$versionstring[1] = trim(phpthumb_functions::SafeExec($commandline.$versionstr));
				if (preg_match('/^Version\: [^0-9]*([\s 0-9\.\:\-Q\/]+) (http|file)\:/i', $versionstring[1], $matches)) {
					$versionstring[0] = $matches[2];
					
				} else {
					
					// 2011-09-02 Altered for GraphicsMagick
					$versionstr = ' -version';
					$this->DebugMessage('ImageMagick version checked with "'.$commandline.$versionstr.'"', __FILE__, __LINE__);
					$versionstring[1] = trim(phpthumb_functions::SafeExec($commandline.$versionstr));
					if (preg_match('/^GraphicsMagick/i', $versionstring[1], $matches)) {
						// Have GraphicsMagick identify itself as ImageMagick 6.3.8
						$versionstring[0] = '6.3.8';//$matches[2];
	...
	function ImageMagickThumbnailToGD() {
		// http://www.imagemagick.org/script/command-line-options.php
			$this->DebugMessage('ImageMagickThumbnailToGD() CALLED', __FILE__, __LINE__);
		...
		if ($commandline) {
			if ($IMtempfilename = $this->phpThumb_tempnam()) {
				...
				clearstatcache();
				if (isset($IMtempSourceFilename) && file_exists($IMtempSourceFilename)) {
